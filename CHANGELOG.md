...
- **SDK Upgrade**: Upgraded `@ansango/lastfm-api` to `^3.7.1`, picking up the corrected `user.*` pagination `@attr` field names (`page`/`perPage`) and the removal of the unsupported `page` parameter on `user.getTopTags`.

## [0.9.0] — 2026-08-30

### Changed

- **SDK Upgrade**: Upgraded `@ansango/lastfm-api` to `^3.7.0` leveraging the new modular layered architecture and performance improvements.
