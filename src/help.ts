...
export function generalHelp(): string {
  return [
    ...
    ``,
    `Commands:`,
    `  watch          live real-time scrobble watcher and now-playing monitor`,
    `  reports        automated reports (wrapped, milestones, monthly digest)`,
    `  playlists      smart playlists (generate, export-m3u, export-csv)`,
    `  exporter       bulk export (scrobbles, loved, library)`,
    `  insights       20 analytical views (summary, mood, obscurity, streaks, etc.)`,
    ``,
    `Examples:`,
    `  lastfm watch --user ansango`,
    `  lastfm reports wrapped --user ansango --year 2025`,
    `  lastfm playlists generate --user ansango --mode heavy-rotation`,
    `  lastfm exporter scrobbles --user ansango --format csv > scrobbles.csv`,
    `  lastfm insights obscurity --user ansango`,
